...
import { getAgents, addAgent, updateAgent, deleteAgent,getAgentInfo } from '@/services/agent';
...
const CustomerEdit = () => {
  ...
  const fetchAgentInfoData = async () => {
    try {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await updateAgent({ uuid, ...values });
      if (res.code !== 200) {

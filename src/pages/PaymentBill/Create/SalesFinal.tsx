...
import {createFinalPaymentBill} from '@/services/sales_order';
...
const PaymentBillCreate = () => {
  ...
  const fetchAgreementByOrder = async (orderNo) => {
    try {
      const response = await getAgreementByOrder({ order_no: orderNo, type: "销售尾款合同" });
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        form.setFieldsValue({
          ...
          amount: response.data.final_payment_amount,
        });
      } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await createFinalPaymentBill(values);
      if (res.code !== 200) {
